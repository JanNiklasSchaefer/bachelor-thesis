...
public class MarioGame {
    ...
    /**
     * Modified the following function to contain the drawDebug boolean to decide if the debug draw should be activated.
     * It was modified as part of Jan Niklas Schäfer's Bachelor Thesis
    ...
    public MarioResult playGame(String level, int timer) {
        return this.runGame(new Agent(), level, timer, 0, true, false, 2, 30);
    }
    ...
    public MarioResult playGame(String level, int timer, int marioState) {
        return this.runGame(new Agent(), level, timer, marioState, true, false, 2, 30);
    }
    ...
    public MarioResult playGame(String level, int timer, int marioState, int fps) {
        return this.runGame(new Agent(), level, timer, marioState, true, false, 2, fps);
    }
    ...
    public MarioResult playGame(String level, int timer, int marioState, int fps, float scale) {
        return this.runGame(new Agent(), level, timer, marioState, true, false, scale, fps);
    }
    ...
    public MarioResult runGame(MarioAgent agent, String level, int timer) {
        return this.runGame(agent, level, timer, 0, false, false, 2, 0);
    }
    ...
    public MarioResult runGame(MarioAgent agent, String level, int timer, int marioState) {
        return this.runGame(agent, level, timer, marioState, false, false, 2, 0);
    }
    ...
    public MarioResult runGame(MarioAgent agent, String level, int timer, int marioState, boolean visuals) {
        return this.runGame(agent, level, timer, marioState, visuals, false, 2, visuals ? 30 : 0);
    }
     ...
     * @return statistics about the current game
     */
    public MarioResult runGame(MarioAgent agent, String level, int timer, int marioState, boolean visuals, boolean drawDebug) {
        return this.runGame(agent, level, timer, marioState, visuals, drawDebug, 2, visuals ? 30 : 0);
    }
    ...
    public MarioResult runGame(MarioAgent agent, String level, int timer, int marioState, boolean visuals, int fps) {
        return this.runGame(agent, level, timer, marioState, visuals, false, 2, fps);
    }
     ...
     * @param marioState the initial state that mario appears in. 0 small mario, 1 large mario, and 2 fire mario.
     * @param visuals    show the game visuals if it is true and false otherwise
     * @param drawDebug  Enable DrawDebug to drawLines like in Robin Baumgarten A* Agent Video
     * @param scale      the screen scale, that scale value is multiplied by the actual width and height
     * @param fps        the number of frames per second that the update function is following
     * @return statistics about the current game
     */
    public MarioResult runGame(MarioAgent agent, String level, int timer, int marioState, boolean visuals, boolean drawDebug, float scale, int fps) {
        if (visuals) {
        ...
        this.setAgent(agent);
        return this.gameLoop(level, timer, marioState, visuals, fps, drawDebug);
    }

    /**
     * Modified the following function to contain the drawDebug Array as part of Jan Niklas Schäfer's Bachelor Thesis
     * Author: Jan Niklas Schäfer
     */
    private MarioResult gameLoop(String level, int timer, int marioState, boolean visual, int fps, boolean drawDebug) {
        ...
        while (this.world.gameStatus == GameStatus.RUNNING) {
            if (!this.pause) {
                ...
                //Added drawCoordinate to get Actions function. Author: Jan Niklas Schäfer
                boolean[] actions = this.agent.getActions(new MarioForwardModel(this.world.clone()), agentTimer, this.world.drawCoordinates);
                ...
                /**
                 * The following lines of code were added as part of Jan Niklas Schäfer's bachelor thesis
                 * Author: Jan Niklas Schäfer
                 * Added overtime calculation. Average out calculation time over time. If we have a positive value substract from overtime.
                 * If it is negative and bigger than grace period add onto our time. This allows to check if algorithm adheres to 40ms rule
                 **/
                if (agentTimer.getRemainingTime() < 0 && Math.abs(agentTimer.getRemainingTime()) > MarioGame.graceTime) {
                    this.world.overtime += (int) Math.abs(agentTimer.getRemainingTime());
                } else if (agentTimer.getRemainingTime() > 0) {
                    this.world.overtime -= (int) Math.abs(agentTimer.getRemainingTime());
                }
                /**
            ...
            //render world
            if (visual) {
                // Added drawDebug Boolean to enable toggle of debugDraw. Author: Jan Niklas Schäfer
                this.render.renderWorld(this.world, renderTarget, backBuffer, currentBuffer, drawDebug);
                //End of code modification
            }
